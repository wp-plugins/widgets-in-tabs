...
class Widgets_In_Tabs extends WP_Widget {
	...
	public function enqueue_scripts() {
		wp_register_script('wit', plugins_url( 'wit-all.min.js', __FILE__ ), array(
			'jquery',
	...
	public function enqueue_admin_scripts($hook) {
		if ('widgets.php' != $hook)
			return;
		wp_register_script('wit-admin', plugins_url( 'wit-admin.min.js', __FILE__ ), array('jquery'), WIT_VERSION, true);
		wp_enqueue_script( 'wit-admin' );
	}
	...
	public function form( $instance ) {
		...
		<p>
			<legend><?php _e( 'Tabs bar style:' , 'wit'); ?></legend>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'interval' ); ?>"><?php _e( 'Auto-Rotate tabs every: (0 = disable)' , 'wit'); ?></label> 
		...
		<div>
			...
			<div id="<?php echo $this->get_field_id( 'wit-extra-controls' ); ?>" <?php if ($instance['extra'] === 'false') echo 'style="display: none"'; ?>>
				<p>
					...
					<label for="<?php echo $this->get_field_id( 'wit_extra_closable_tabs' ); ?>"> <?php _e('Hide current tab if its title is clicked', 'wit') ?></label>
				</p>
				<p <?php if ($instance['extra_closable_tabs'] === 'false') echo 'style="display: none;"'; ?>>
					...
					<label for="<?php echo $this->get_field_id( 'wit_extra_closable_tabs_init' ); ?>"> <?php _e('Also, start with all tabs hidden', 'wit') ?></label>
				</p>
				<p>
